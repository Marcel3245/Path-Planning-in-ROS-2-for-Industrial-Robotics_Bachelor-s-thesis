...
def generate_launch_description():
    ...
    # General Path
    package_name_ros2 = 'mycobot_ros2'
    ...
    ros2_path = FindPackageShare(package=package_name_ros2).find(package_name_ros2)
   ...
   ## ================================================================ RVIZ INITIALIZATION ================================================================= ##
    rviz_config_path = os.path.join(ros2_path, 'rviz', 'rviz.rviz')
    rviz2_node = TimerAction(
        period=6.0,  # Longer than the jsb_spawner timer
        actions=[
            Node(
                package='rviz2',
                executable='rviz2',
    ...
    belt_movement = Node(
        package='mycobot_conveyor_belt',
        executable='conveyor_belt',
        name='conveyor_belt',
        output='screen',
        parameters=[{"use_sim_time": LaunchConfiguration("use_sim")}],
    )
    hmi = Node(
        package='mycobot_interface',
        executable='hmi',
        name='hmi',
        output='screen',
        parameters=[{"use_sim_time": LaunchConfiguration("use_sim")}],
